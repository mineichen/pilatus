...
use crate::image::{
    protocol::{calculate_buf_len, KIND_IMAGE},
    DataType, ImageWithMeta, StreamImageError,
};
...
fn extract_meta_and_image(
    input: &[u8],
) -> Result<Result<ImageWithMeta<DynamicImage>, StreamImageError<DynamicImage>>, anyhow::Error> {
    let meta_content_len = u32::from_le_bytes(array(&input[4..8]));
    let _meta_bytes = input.get(8..meta_content_len as usize + 8).ok_or_else(|| {
        anyhow!(
    ...
    anyhow::ensure!(Some(&KIND_IMAGE) == input.get(after_meta));
    let (image, rest) = read_image(&input[after_meta + 1..])?;
    let mut input = rest;
    let meta_image = ImageWithMeta::with_hash(image, None);
    loop {
        match input {
...
fn read_mask(input: &[u8]) -> anyhow::Result<((), &[u8])> {
    Ok(((), input))
}
