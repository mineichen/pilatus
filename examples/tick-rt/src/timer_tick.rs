...
use pilatus_tick::TimerTickParams;
use tracing::info;
...
) -> Result<TimerTickParams, UpdateParamsMessageError> {
    ctx.params_as::<TimerTickParams>()
}

async fn device(
    ctx: DeviceContext,
    params: TimerTickParams,
...
impl State {
    async fn update_params(
        &mut self,
        msg: UpdateParamsMessage<TimerTickParams>,
    ) -> ActorResult<UpdateParamsMessage<TimerTickParams>> {
...
struct UpdateTickMessage(Weak<TimerTickParams>);
...
pub fn create_default_device_config() -> pilatus::DeviceConfig {
    pilatus::DeviceConfig::new_unchecked(DEVICE_TYPE, DEVICE_TYPE, TimerTickParams::default())
}
